...
| **Estorno e ajuste contábil** | Candidato natural à v3 — o ledger append-only da F1 já sustenta o modelo. |
...
Em ordem de proximidade com o que a v2 constrói:

1. **Estorno** — transação de sinal oposto vinculada à original. O ledger já suporta; falta o vínculo e a regra de janela.
2. **Multi-agência** — `agencia` já é coluna, hoje com valor fixo `0001`.
3. **Limite de saque diário** — política pura, entra em `core/` sem tocar no schema. Bom exercício da fronteira de [DT-05](01-decisoes-tecnicas.md#dt-05).
4. **Conta conjunta** — quebra o `cliente_id` único de `contas` e vira uma tabela de titulares. Mudança de modelagem, não de feature.
5. **Notificação de movimentação** — exige fila ou webhook; primeira dependência de infraestrutura assíncrona do projeto.
